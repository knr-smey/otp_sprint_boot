-- Initial schema: authentication foundation + TOTP two-factor support (H2 variant for tests).
-- Mirrors the MySQL migration; existing behaviour is unaffected:
-- every user starts with two_factor_enabled = FALSE.
...
CREATE TABLE user_two_factor_auth (
    ...
    pending_secret_enc VARCHAR(512),
    updated_at TIMESTAMP(6) NOT NULL,
    PRIMARY KEY (user_id),
